...
# Read the data file from "https://blobserver.dc.scilifelab.se/blob/wastewater_data_Umeaa.xlsx"
# and extract the relevant data
wastewater_data = pd.read_excel(
    "https://blobserver.dc.scilifelab.se/blob/wastewater_data_Umeaa.xlsx",
    engine="openpyxl",
    usecols=["Faktiskt vecka", "Medelvärde per vecka"],
)
...
# Drop the first 7 rows (no data) to start at week 44
wastewater_data = wastewater_data.drop(wastewater_data.index[:8])
...
def format_week(week_num, index):
    ...
    if index in [8, 9, 21, 27, 28, 29, 30, 31]:
        year_week += "*"

    # Add "**" to weeks with discrepancy in data collection
    if index in [20, 27, 28, 29, 30, 31]:
        year_week += "**"
...
pio.write_json(fig, "wastewater_graph_Umea.json")
