...
df["authors"] = np.where(
    df["authorscount"] == 2, df["fauth"] + " and " + df["sauth"], df["fauth"]
)
df["Authors"] = np.where(df["authorscount"] > 2, df["fauth"] + " et al.", df["authors"])
df["Altmetrics"] = "altmetrics"
df = df[["Published", "Authors", "Title"]]
...
fig = go.Figure(
    data=[
        go.Table(
            ...
            cells=dict(
                values=(df["Published"], df["Authors"], df["Title"]),
                align=["center", "left", "left"],
                fill_color=[[oddrowcol, evenrowcol] * 10],
                font=dict(color=bodytextcol, size=12),
                height=30,
                line=dict(width=0),
            ),
        )
...
# to show directly in browser. NB- write functions wont work if active
# fig.show()
