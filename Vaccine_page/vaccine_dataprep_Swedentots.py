...
def get_vaccination_data(data_url, sheet_name, set_date=False, only_sweden=False, needed_columns=None):
    df_vacc = pd.read_excel(data_url, sheet_name=sheet_name, header=0, engine="openpyxl", keep_default_na=False)
    
    # if needed, convert week date to full date, initially set day as Monday
    if set_date:
        df_vacc["day"] = 1
        df_vacc["date"] = df_vacc.apply(lambda row: dt.fromisocalendar(row["År"], row["Vecka"], row["day"]), axis=1)
    
    # We need to calculate values for the population. Don't use the values for just the population eligible for vaccination
    df_vacc["Andel vaccinerade"] = df_vacc["Andel vaccinerade"].replace(",", ".", regex=True)
...
first_two_timeseries_lan = get_vaccination_data(
    ...
    set_date=True,
    needed_columns=["date",  "Region", "Antal vaccinerade", "Procent vaccinerade", "Vaccinationsstatus"]
    )
first_two_timeseries = first_two_timeseries_lan[first_two_timeseries_lan["Region"] == "Sweden"]

third_timseries = get_vaccination_data(
    data_url=data_url,
    sheet_name="Vaccinerade tidsserie dos 3",
    set_date=True,
    only_sweden=True,
    needed_columns=["date",  "Region", "Antal vaccinerade", "Procent vaccinerade"]
    )

fourth_timseries = get_vaccination_data(
    data_url=data_url,
...
first_two_vacc_dose_lan = get_vaccination_data(
    data_url=data_url,
    sheet_name="Vaccinerade ålder"
    )
first_two_vacc_dose = first_two_vacc_dose_lan[first_two_vacc_dose_lan["Region"] == "Sweden"]
