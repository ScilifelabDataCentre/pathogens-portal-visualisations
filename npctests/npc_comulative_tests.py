...
cum_data.insert(
    len(cum_data.columns),
    "cumulative sum",
    cum_data.groupby("class")["count"].cumsum()
)
# print(cum_data)
cum_data['date'] = cum_data['date'].dt.strftime('%Y-%m-%d')

plot_npc_cumulative_tests = px.line(cum_data, x='date', y='cumulative sum', color='class', 
...
pio.write_json(plot_npc_cumulative_tests, 'npc_comulative_tests.json')
